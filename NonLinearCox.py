...
df['cluster_' + target] = np.where(df[target] <= -0.42, 1, df['cluster_' + target])
target_names = ['SKM Preserved','SKM Loss']

# cluster_num = 3
# df['cluster_' + target] = 0
# df['cluster_' + target] = np.where(df[target] < -0.29, 1, df['cluster_' + target])
# df['cluster_' + target] = np.where(df[target] > 0.32, 2, df['cluster_' + target])
...
#     mask = (ML_range < -0.29) if i == 1 else (ML_range > 0.32) if i == 2 else (ML_range >= -0.29) & (ML_range <= 0.32)
...
plt.figure(figsize=(8, 6))
for i in range(cluster_num):
    mask = (ML_range <= -0.42) if i == 1 else (ML_range > -0.42)
    plt.plot(ML_range[mask], partial_hazard[mask], label=target_names[i])
