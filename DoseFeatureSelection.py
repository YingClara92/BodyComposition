...
#
# selected_feature = data.columns[5:].tolist()
...
data['cluster_' + target] = np.where(data['speed_'+target] < -0.42, 1, data['cluster_' + target])
...
for cluster in cluster_num:
    ...
    for fea in selected_feature:
        ...
        p_value = result.pvals_[0]/len(selected_feature)
...
## use VIF to iteratively remove features with high multicollinearity
...
print(vif_data)
## iterative remove the feature with the highest VIF until all VIF are below 5
